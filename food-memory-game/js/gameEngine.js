...
class GameEngine {
    ...
    showInvestigationResults(newCluesFound, clues) {
        const resultText = newCluesFound > 1
            ? `Excellent detective work! I found ${newCluesFound} new pieces of evidence:`
            : `Great! I discovered 1 new piece of evidence:`;

        const cluesList = clues.map(clue => `• ${clue.title}`).join('\n');

        if (window.gameDialogSystem) {
            window.gameDialogSystem.showDialog({
                speaker: "Food Detective",
                text: `${resultText}\n\n${cluesList}\n\nThis evidence might help someone remember their favorite foods! I should talk to the people in town and see if any of these clues help restore their memories.`,
                choices: [
                    { text: "📋 Check my investigation notebook", action: "show_inventory" },
                    { text: "🗣️ Go talk to people in town", action: "close" }
                ]
            });
        }
    }
