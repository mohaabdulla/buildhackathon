...
class DialogSystem {
    ...
    startDialog(npcId, dialogKey = 'initial') {
        ...
        // Show floating hint for first dialog
        if (window.gameMain && this.dialogs.length === 1) {
            window.gameMain.showFloatingHint("Choose your responses carefully - they affect the story!", 3000);
        }
    }
    ...
    useClueForCustomer(customerKey, clueId) {
        ...
        if (success) {
            ...
            if (customerStatus.solved) {
                ...
                this.showDialog({
                    speaker: "Food Detective",
                    text: `Good progress! The evidence helped ${customerKey} remember more about their food preferences. Progress: ${customerStatus.cluesFound}/${customerStatus.cluesNeeded} clues found. I need to find ${customerStatus.cluesNeeded - customerStatus.cluesFound} more ${customerStatus.cluesNeeded - customerStatus.cluesFound === 1 ? 'clue' : 'clues'}.`,
                    choices: [
